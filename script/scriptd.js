...
let fly = new Audio();
let red_breack = new Audio();
let blue_break = new Audio();
let sam_fall = new Audio();


fly.src = "audio/subnautica_below_zero_Slow Burn.mp3";
fly.loop = true;
red_breack.src = "audio/back2.mp3";
blue_break.src = "audio/blue_break2.mp3";
sam_fall.src = "audio/samolet.mp3";
...
function handleMouseUp(event) {
  if (game.status == "waitingReplay") {
    ...
    for (var i=0; i<coinsHolder.coinsInUse.length;i++){
      const ennemy = coinsHolder.coinsInUse[i];
      coinsHolder.mesh.remove(ennemy.mesh);
    }
...
function createLights() {
  ...
  ambientLight = new THREE.AmbientLight(0xff8844, 0.2);
...
var AirPlane = function () {
  ...
  var matchassis1 = new THREE.MeshPhongMaterial({
    color: Colors.blue,
    shading: THREE.FlatShading,
  });
  ...
  wheelTireB.scale.set(0.5, 0.5, 0.5, 0.1);
...
if (d<game.ennemyDistanceTolerance){
  ...
  particlesHolder.spawnParticles(ennemy.mesh.position.clone(), 15, Colors.red, 5)
...
var speed =.6+Math.random()*0.2;
TweenMax.to(this.mesh.rotation, speed, {x:Math.random()*12, y:Math.random()*12});
TweenMax.to(this.mesh.scale, speed, {x:0.1, y:0.1, z:0.1})
TweenMax.to(this.mesh.position, speed, {x:targetX, y:targetY, delay:Math.random()*0.1,
...
Coin = function (){
  ...
  var mat = new THREE.MeshPhongMaterial({color:Colors.blue, shininess:0,specular:0xffffff,shading:THREE.FlatShading});
  this.mesh = new THREE.Mesh(geom, mat);
  this.mesh.castShadow=true;
  this.angle = 0;
  this.dist  = 0;
...
CoinsHolder.prototype.spawnCoins = function () {
  ...
  for (var i=0; i<nCoins; i++){
    ...
    coin.mesh.position.y = -game.seaRadius+Math.sin(coin.adgle)*coin.distance;
...
CoinsHolder.prototype.rotateCoins = function () {
  for (var i=0; i<this.coinsInUse.length;i++){
    ...
    if (coin.angle>Math.PI*2) coin.adgle-=Math.PI*2;
    ...
    if (d<game.coinDistanceTolerance){
      ...
      particlesHolder.spawnParticles(coin.mesh.position.clone(), 5,0x00999, 0.8);
      addEnergy();
      i--;

    }
    else if (coin.adgle>Math.PI){
      this.coinsPool.unshift(this.coinsInUse.splice(i,1)[0]);
      this.mesh.remove(coin.mesh);
      i--;
...
coinsHolder.spawnCoins();
    ...
    coinsHolder.rotateCoins();
    airplane.mesh.position.y -= game.planeFallSpeed * deltaTime * 0.0002;
    sea.moveWaves();
    sea.mesh.rotation.z += 0.005;
    sky.mesh.rotation.z += 0.01;
...
function createAnimatedSmoke() {
  ...
  setInterval(() => {
    ...
    setTimeout(() => scene.remove(smoke), 1000);
  }, 500);
...
window.addEventListener("load", function(){
  window.document.querySelector("button.start-game").addEventListener("click",function(){
    init();
